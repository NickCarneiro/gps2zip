...
app.get('/', function(req, res) {
	//validate lat and lon params
	if(req.query.lat === undefined || req.query.lon === undefined){

		var response = {error: "Must pass lat and lon parameters."}
		res.json(response);

	} else if(isNaN(req.query.lat) || isNaN(req.query.lon)){
		return {error: "Invalid lat or lon parameters"};

	} else{
		var response = gps.gps2zip(req.query.lat, req.query.lon);
		res.json(response);
	}
});
...
app.get('/demo', function(req, res){
    ...
    javascripts: ["http://maps.googleapis.com/maps/api/js?sensor=false", "javascripts/script.js"],
